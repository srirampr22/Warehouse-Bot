...
static bool compareByWeight(mbot_lcm_msgs::particle_t a, mbot_lcm_msgs::particle_t b) {
                                                        ...
                                                        const OccupancyGrid& map)
{
    ...
    if(hasRobotMoved){
        ...
        auto prior = resamplePosteriorDistribution_and_reinvigorate(laser, map);
                                                           ...
                                                           const bool reinvigorate)
{
    ...
    std::cout<<"Only Resampling"<<std::endl;
    // float average_weight = 0.0;
    // for(auto& p: posterior_){
    //     average_weight += p.weight;
    // }
    // // average_weight /= kNumParticles_;
    // std::cout<<"Average weight: "<<average_weight<<std::endl;

    // samplingAugmentation_.insert_average_weight(average_weight);
    // ParticleList prior = resamplePosteriorDistribution(keep_best, reinvigorate);
    ParticleList prior = lowVarianceSample(kNumParticles_, posterior_);

    // for(auto& p: prior){
    //     if(samplingAugmentation_.sample_randomly()){
    //         p = randomPoseGen_.get_particle(map);
    //     }
    // }
    // if(reinvigorate){
                                                           ...
                                                           const bool reinvigorate)
{
    ...
    std::cout<<"Only Resampling without map"<<std::endl;
    if(keep_best){
        ParticleList posterior_sorted = posterior_;
        ...
        int num_particle = static_cast<int>(0.4 * posterior_sorted.size());
        for(int i = 0; i < num_particle; i++){
            prior.push_back(posterior_sorted[i]);
        }
...
ParticleList ParticleFilter::resamplePosteriorDistribution_and_reinvigorate(const mbot_lcm_msgs::lidar_t& laser, const OccupancyGrid& map,
                                                           const bool keep_best,
                                                           bool reinvigorate)
{
    //////////// TODO: Implement your algorithm for resampling from the posterior distribution ///////////////////
    ...
    std::sort(prior.begin(),prior.end(), compareByWeight);
    double bot_avg = 0;
    ...
    //reuse garbage particles|| homogenous exploration || jerk_filter
    if (reinvigorate){
            std::random_device rd;
            std::mt19937 e2(rd());
    ...
    // if(reinvigorate){
    //     reinvigoratePriorDistribution_stratergy2(posterior_, laser, map);
    // }
    return prior;
}
...
void ParticleFilter::reinvigoratePriorDistribution(ParticleList& prior)
{
    ...
    // std::cout<<"Distribution quality: "<<distribution_quality<<std::endl;

    // Sort the particles in the posterior based on weight
    // std::sort(prior.begin(), prior.end(), [](const Particle& a, const Particle& b) {
    //               return a.weight < b.weight; 
    //           });
    ...
    // distribution_quality = 0.0;
    // size_t bottom_40_percent_index = static_cast<size_t>(1.0 * prior.size());
    // for(size_t i = 0; i < bottom_40_percent_index; ++i){
    //     distribution_quality += prior[i].weight;
    // }

    distribution_quality /= kNumParticles_;

    std::cout << "Distribution quality: " << distribution_quality <<" "<< "of bottom 40% :"<< kNumParticles_ << std::endl;
