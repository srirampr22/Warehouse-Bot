...
private:
    //float fwd_pid[3] = {1.0, 0.0, 0.0};
    float fwd_pid[3] = {3.0, 0.0, 0.0};
    float fwd_sum_error = 0;
    float fwd_last_error = 0;
    //float turn_pid[3] = {3.0, 0.0, 0.0};
    float turn_pid[3] = {10.0, 0.0, 0.0};
...
private:
    ...
    float turn_pid[3] = {20, 0.005, 0.07}; 
    float turn_sum_error = 0;
    float turn_last_error = 0;
    float turn_sum_error_inplace = 0; 
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        this->turn_sum_error += d_theta;
        float turn_der = 0.0;

        // Update derivative of angle error
        // if (turn_last_error > 0)
        //     turn_der = (d_theta - turn_last_error) / 0.05;

        turn_der = (d_theta - turn_last_error) / 0.05;
        
        // NOTE integral error may need to be zero'd out
        float turn_vel = turn_pid[0] * d_theta + turn_pid[1] * turn_sum_error + turn_pid[2] * turn_der;
        // fprintf(stdout,"Turn error: %f\tTurn vel: %f\tPose theta: %f\n", d_theta, turn_vel, pose.theta);

        // Update last turn error for next iteration 
        this->turn_last_error = d_theta; 
    ...
    mbot_lcm_msgs::twist2D_t get_command_final_turn(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target)
    {
        ...
        // PID for the angular velocity given the delta theta
        turn_sum_error_inplace += d_theta;
        float turn_der = 0;
        if (turn_last_error > 0)
            turn_der = (d_theta - turn_last_error) / 0.05;
        ...
        float turn_vel = turn_pid[0] * d_theta + turn_pid[1] * turn_sum_error_inplace + turn_pid[2] * turn_der;
        // fprintf(stdout,"Turn error: %f\tTurn vel: %f\tPose theta: %f\n", d_theta, turn_vel, pose.theta);
    ...
    virtual bool target_reached(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target, bool is_end_pose)  override
    {
        ...
        bool target_reached_condition = fabs(angle_diff(pose.theta, target_heading)) < 0.02;

        if (target_reached_condition == true) { 
            turn_sum_error = 0;  
        }

        return target_reached_condition;
    ...
    bool target_reached_final_turn(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target)
    {
        ...
        bool target_reached_condition = (fabs(angle_diff(target.theta, pose.theta)) < 0.02);

        if (target_reached_condition == true) {
            turn_sum_error_inplace = 0; 
        }
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        if (fabs(alpha) > (M_PI_4 / 2.0))
        {
            fwd_vel = 0;
        }

        printf("FWD Vel: %f, Turn Vel: %f\n", fwd_vel, turn_vel);
        return {0, fwd_vel, 0, turn_vel};
    }
    ...
    */
    mbot_lcm_msgs::twist2D_t updateCommand(void) 
    {
        ...
        if(!targets_.empty() && !odomTrace_.empty()) 
        {
            ...
            std::cout<<"State of motion controller: ";

            if (state_ == INITIAL_TURN) {
                std::cout << "INITIAL_TURN";
            ...
            std::cout << " CURRENT TARGET : " << "(" << target.x << ", " << target.y << ", " << target.theta << ")"; 
            std::cout << std::endl;

            // Check to see if we are turning in place 
            float pose_error_x = target.x - pose.x; 
            float pose_error_y = target.y - pose.y; 
            float pose_error_theta = target.theta - pose.theta;
            if (fabs(pose_error_x) < 0.01 && fabs(pose_error_y) < 0.01 && (fabs(pose_error_theta) > (M_PI / 64.0))) { 
                state_ = FINAL_TURN;
            }
    ...
    {
        newpath_ = path->path;
        std::reverse(newpath_.begin(), newpath_.end()); // store first at back to allow for easy pop_back()

        // Check to see if the final goal for a new path is the same as the current path being run. 
        // If it is, ignore it. 
        if(!targets_.empty()){
            if((newpath_[0].x == targets_[0].x) && (newpath_[0].y == targets_[0].y) && (newpath_[0].theta == targets_[0].theta)){
                return;
            }
        }
        targets_ = newpath_;
        ...
        state_ = INITIAL_TURN;
        // Pop off if first pose is current pose 
        // mbot_lcm_msgs::pose2D_t currentTarget = targets_.back(); 
        // mbot_lcm_msgs::pose2D_t currentPose = currentPose(); 

        // if (currentTarget.x == currentPose.x && currentTarget.y == currentPose.y && currentTarget.theta == currentPose.theta) {
        //     assignNextTarget();
        // }
...
private:
    ...
    std::vector<mbot_lcm_msgs::pose2D_t> newpath_;
...
int main(int argc, char** argv)
{
    ...
    while(true)
    {
    	...
    	if(controller.timesync_initialized()){
            ...
            if (cmd.vx > max_lin_vel) cmd.vx = max_lin_vel;
            else if (cmd.vx < -max_lin_vel) cmd.vx = -max_lin_vel;

            // Angular vel
            float max_ang_vel = M_PI / 3.0;
            // float max_ang_vel = M_PI_4;
