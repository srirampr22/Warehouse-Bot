                                     ...
                                     const ParticleList& particles)
{
  ...
  while (samples.size() < num_particles)
  {
    ...
    while (sum < r && idx < particles.size() - 1) {
      ++idx;
                                      ...
                                      const ParticleList& particles)
{
    ...
    for (int i = 0; i < num_particles; ++i)
    {
        ...
        while (u > s && idx < particles.size() - 1) {
            ++idx;
...
private:
    ...
    ParticleList resamplePosteriorDistribution_and_reinvigorate(const mbot_lcm_msgs::lidar_t& laser, 
                                                            const OccupancyGrid& map,
